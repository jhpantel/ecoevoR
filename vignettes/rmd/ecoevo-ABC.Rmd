...
### Box 2A
pdf(file=file.path(datapath,"Box_2a.pdf"),width=((12*90)/368),height=((7*90)/368),pointsize=6)
par(mfrow=c(1,2),mar = c(2, 2, 0.5, 0))
...
pdf(file=file.path(datapath,"Box_2c.pdf"),width=13.3,height=13.3)
par(mfcol=c(4,4),mar = c(2, 2, 0.5, 0))
for(i in 1:16){
  if (max(abc_sim_summ_x0[choice[i],1:66]) == 0){
    lim <- 10
  ...
  matplot(days,cbind(abc_sim_summ_x0[choice[i],1:22],abc_sim_summ_x0[choice[i],23:44],abc_sim_summ_x0[choice[i],45:66]),type="l",xlab="time",ylab="N", col=c("black","black","black"), lty=1, ann=F, axes=F, ylim=c(0,lim), lwd=1.5)
  axis(1,at=c(0,50,100,150,200,250,300),labels=FALSE,lwd=4)
  axis(2,at=c(0,quantile(0:lim,probs=c(.25,.5,.75)),lim),labels=FALSE,lwd=4)
  # axis(2,at=c(0,quantile(0:lim,probs=c(.25,.5,.75)),lim),labels=c("0","",quantile(0:lim,probs=c(.5)),"",lim),lwd=4)
  points(days,abc_sim_summ_x0[choice[i],1:22],pch=19,col="grey",cex=2)
  points(days,abc_sim_summ_x0[choice[i],23:44],pch=19,col="skyblue",cex=2)
  points(days,abc_sim_summ_x0[choice[i],45:66],pch=19,col="orange",cex=2)
}
dev.off()
...
pdf(file=file.path(datapath,"Box_2e.pdf"),width=((12*90)/368),height=(((7*90)/368)*1.5),pointsize=6)
