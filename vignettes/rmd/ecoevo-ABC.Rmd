...
### Box 2A
pdf(file=file.path(datapath,"Box_2a.pdf"),width=7.87402,height=2.11854331,pointsize=8)
par(mfrow=c(1,2),mar = c(2, 2, 0.5, 0))
par(mai=c(0.4,0.4,0.2,0))
matplot(1:300,sim1[,7:9], type="l",xlab="time",ylab="N", col=c("grey","skyblue","orange"), lty=1, lwd=1.25, ann=F, axes=F)
axis(1,at=c(0,50,100,150,200,250,300),labels=c(0,"",100,"",200,"",300),lwd=2)
axis(2,at=c(0,200,400,600,800),lwd=2)
points(days,sim1[days,7],pch=19,col="grey",cex=1.5)
points(days,sim1[days,8],pch=19,col="skyblue",cex=1.5)
...
pdf(file=file.path(datapath,"Box_2c.pdf"),width=7.93,height=1.69484252,pointsize=10)
par(mfcol=c(2,6),mar = c(2, 2, 0.5, 0))
for(i in 1:12){
  if (max(abc_sim_summ_x0[choice[i],1:66]) == 0){
    lim <- 10
