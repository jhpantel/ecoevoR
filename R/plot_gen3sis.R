...
#' max_alpha <- max(output$summary$`richness-final`[,3],na.rm=TRUE)
...
plot_gen3sis <- function (output, summary_title = NULL, summary_legend = NULL, max_gam=500, max_alpha=100)
{
  ...
  {
    graphics::layout(matrix(c(1, 3, 3, 1, 3, 3, 1, 3, 3, 2, 2, 2, 2, 2, 2), ncol = 3, byrow = TRUE))
    graphics::par(mar = c(4, 3, 3, 7), oma = c(0.1, 0.8, 0.3, 0.8))
    graphics::plot(1, type = "n", xlab = "", ylab = "", xlim = c(0,10), ylim = c(0, 10), axes = FALSE, ann = FALSE)
    if (is.null(summary_title)) {
      summary_title <- stringr::str_split(output$parameters$directories$input,"/")[[1]]
      summary_title <- paste(raster::tail(summary_title, 2)[1],
                             collapse = ">")
    }
    ...
    graphics::points(d[, "extinctions"], pch = 4, col = grDevices::rgb(1, 0, 0, 0.5), type = "b")
    graphics::axis(2, line = -1, cex = 1, cex.axis = 1, col = "black")
    graphics::mtext(side = 2, text = "Evolutionary events", col = "black", line = 1.5, cex = 1.1)
    graphics::legend(x = 1, y = 80, legend = c("Richness", "Speciation", "Extinction"), col = c("black", grDevices::rgb(0, 0, 1, 0.5), grDevices::rgb(1, 0, 0, 0.5)), pch = c(NA, 3, 4), lty = c(1, NA, NA), lwd = c(4, NA, NA), bty = "n")
    axis_lab <- seq(from = 1, to = nrow(d), length.out = max((nrow(d)/20), 2))
    graphics::axis(1, at = axis_lab, labels = rownames(d)[axis_lab])
    graphics::mtext(side = 1, text = "Time steps", line = 2.5, cex = 1.1)
    ras <- raster::rasterFromXYZ(output$summary$`richness-final`)
    ...
    if (max_ras == 0) {
      ...
      rc <- gen3sis::color_richness(max_ras)
    }
    if (min_ras == 0) {
    ...
    raster::plot(raster::rasterFromXYZ(output$summary$`richness-final`),breaks=breaks,legend.only = TRUE, add = TRUE, col = cols, axis.args=arg)
  }
}
